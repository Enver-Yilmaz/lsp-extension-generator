...
/**
 * Summary. Instance of the example2Lexer class.
 *
 * Description. This variable represents an instance of the example2Lexer class which contains all the tokens
 * identified in the given grammar and also operations to check lexical errors on a given text.
 *
 * @author Sergio
 * 
 */
const example2Lexer = require('./extension/node_service/antlr/example2Lexer').example2Lexer;

/**
 * Description. This variable represents an instance of the example2Lexer class which contains all the rules
 * identified in the given grammar and also operations to check syntactical errors on a given text.
 *
 * @author Sergio
 * 
 */
const example2Parser = require('./extension/node_service/antlr/example2Parser').example2Parser;
...
fp(3000, function (err, freePort) {
    ...
    var lexer = new example2Lexer(inputStream);
    var commonTokenStream = new antlr.CommonTokenStream(lexer);
    var parser = new example2Parser(commonTokenStream);
    extension = '';
    for (value of array) {
    ...
    fs.readFile(homedir + '/Code/User/settings.json', function (err, data) {
        ...
        //var associations = JSON.parse(left + ":" + right);
        ...
        /*if (jsonText["files.associations"].length !== 0) {
            jsonText["files.associations"] += ',';
        }
        jsonText["files.associations"] += JSON.stringify(associations);*/
